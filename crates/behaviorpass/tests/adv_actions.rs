...
fn actions_just_carries_the_exact_become_with_no_effects() {
    let a = Actions::<Never, u64, u32>::just(Step::Continue);
    assert_eq!(a.become_, Step::Continue);
    assert!(a.sends.is_empty());
    assert!(a.creates.is_empty());

    let b = Actions::<i32, u64, u32>::just(Step::Goto(7));
...
fn actions_cont_is_pure_continue() {
    let a = Actions::<Never, u64, u32>::cont();
    assert_eq!(a.become_, Step::Continue);
...
fn actions_stop_carries_the_exit_verbatim() {
    for exit in [Exit::Normal, Exit::Collected, Exit::LinkDied(0), Exit::LinkDied(u64::MAX)] {
        let a = Actions::<Never, u64, u32>::stop(exit);
...
fn actions_goto_carries_the_phase_verbatim() {
    let a = Actions::<i32, u64, u32>::goto(-3);
...
fn mailaddr_is_an_opaque_copyable_token() {
    for addr in [MailAddr(0), MailAddr(1), MailAddr(u64::MAX)] {
        ...
        let actions = Actions::<Never, u64, u32> {
            sends: vec![(addr, 7)],
            creates: Vec::new(),
            become_: Step::Continue,
        };
        assert_eq!(actions.sends, vec![(addr, 7)], "the boundary address survives the trace");
    }
}
