#![no_main]
//! Arbitrary keyed assignment delivery, completion, retirement, and shutdown order.
...
use behavior::atomic::{
    ...
    KeyedAssignedReturnReason, KeyedCommand, KeyedEvent, KeyedOutcome, KeyedPool,
...
fn submit_work(pool: &mut SearchPool) -> AcceptedWork {
    let customer =
        Recipient::<MessageProtocol<RuntimeAddress, KeyedOutcome<Account, Role, u8, u16>>>::global(
            RuntimeAddress(88),
        );
    let acted = pool
        .receive(
            RuntimeAddress(7),
            KeyedCommand::submit(SubmissionId::new(41), Account(2), 37, customer),
        )
        .unwrap_or_else(|_| panic!("ready worker accepts one keyed job"));
    let accepted = acted
        .sends
        .customer_outcomes
        .into_requests()
    ...
    let (job, binding) = match accepted {
        CustomerDelivery::Logical { delivery } => match delivery.message {
            KeyedOutcome::Accepted {
                ...
                assert_eq!(submission, SubmissionId::new(41));
                assert_eq!(binding.role(), &Role::Search);
                (job.get(), binding)
            }
            _ => panic!("ready admission is accepted"),
        },
        CustomerDelivery::Established { .. }
        | CustomerDelivery::RejectedLogical { .. }
        | CustomerDelivery::RejectedEstablished { .. } => panic!("logical route changed"),
    };
...
impl Scenario {
    fn apply(self, input: FuzzInput) -> Self {
        if let PoolProgress::PoolStopped = self.progress {
            return self;
        }
        let Self {
            mut pool,
        ...
        let (delivery, selected) = match (delivery, input) {
            ...
            (DeliveryCustody::Emitted(action), FuzzInput::RejectDelivery) => {
                ...
                (
                    ...
                    Some((
                        expected,
                        pool.transition(KeyedEvent::AssignmentSettled(rejected)),
                    )),
                )
            }
            ...
            (delivery, FuzzInput::Shutdown) => (
                ...
                Some((
                    CustomerEmission::MayFinish,
                    pool.receive(RuntimeAddress(7), KeyedCommand::shutdown()),
                )),
            ),
        ...
        let acted = acted.unwrap_or_else(|_| panic!("every selected keyed input is total"));
        ...
        assert!(acted.sends.diagnostics.len() <= 2);

        let outcomes = acted.sends.customer_outcomes.into_requests();
        match expected {
        ...
        for outcome in outcomes {
            ...
            let terminal = match outcome {
                ...
                KeyedOutcome::ReturnedQueued {
                    ...
                    assert!(matches!(
                        reason,
                        KeyedQueuedReturnReason::PoolShutdown
                            | KeyedQueuedReturnReason::RolePermanentlyUnavailable
                            | KeyedQueuedReturnReason::NoRecoverableWorkers
                            | KeyedQueuedReturnReason::AssignmentCorrelationExhausted
                    ));
                    job.get()
                }
                KeyedOutcome::ReturnedAssigned {
                    ...
                    assert!(matches!(
                        reason,
                        KeyedAssignedReturnReason::PoolShutdown
                ...
                KeyedOutcome::Accepted { .. } | KeyedOutcome::Rejected { .. } => {
        ...
        let progress = match (progress, input, acted.become_) {
            ...
            (PoolProgress::PoolStopped, _, Step::Continue) => PoolProgress::PoolStopped,
        };
        Self {
            pool,
            worker,
