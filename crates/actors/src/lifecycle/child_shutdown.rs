...
use behavior::{
    Actions, Address, Behavior, BirthMode, ChildChoice, ChildHead, ChildOccurrenceProduct,
    ChildOccurrenceShape, ChildOccurrences, ChildTail, CreationId, CreationKind, EventLayer,
    InterpreterRequests, Never, ResolveChildOccurrence, SendEffects, SendLayer, Step,
};
...
pub enum ChildCreationExpectation {
    ...
    Established { creation: CreationId },
...
pub enum ChildShutdownPlanError<E, A: Address> {
    ...
    UnexpectedCreationResult {
        position: usize,
        expected: ChildCreationExpectation,
    ...
    #[error(transparent)]
    InvalidPlan(ShutdownPlanError<CreationId>),
...
mod tests {
    use super::*;
    use crate::{Activate as _, ShutdownCoordinatorError, StopOnShutdown};
    use behavior::{
        BehaviorActed, BehaviorBase, Births, ChildOccurrence, ChildRole, Children, CreateChild,
        CreationSequence, Creations, DeclaredChildOccurrence, Here, Inside, MailAddr, NoBirths,
        NoSends, User,
    };
    ...
    #[derive(Clone, Copy)]
    struct ApplicationChildren {
        store: CreationId,
        gateway: CreationId,
        alternate_store: CreationId,
    }

    impl ApplicationChildren {
        fn issue() -> Self {
            ...
            let alternate_store = creations
                .issue()
                .expect("the alternate store creation ID exists");
            Self {
                store,
                gateway,
                alternate_store,
    ...
    impl Application {
        const fn complete(children: ApplicationChildren) -> Self {
        ...
        const fn missing_store(children: ApplicationChildren) -> Self {
            Self(InitialChildren::MissingStore {
                gateway: children.gateway,
            })
        }

        const fn duplicate_store(children: ApplicationChildren) -> Self {
            Self(InitialChildren::DuplicateStore {
                store: children.store,
                alternate_store: children.alternate_store,
                gateway: children.gateway,
            })
        }
    }
        ...
        fn init(&mut self, _: crate::InitializationTurn) -> BehaviorActed<Self> {
            let creates = match self.0 {
                InitialChildren::Complete { store, gateway } => Children::<MailAddr>::new()
                    ...
                    .child(gateway, StopOnShutdown::new(Gateway))
                    .into_creates(),
                InitialChildren::MissingStore { gateway } => Creations::one(CreateChild::birth(
                    gateway,
                ...
                InitialChildren::DuplicateStore {
                    ...
                    let store_child =
                        || ChildChoice::Tail(ChildChoice::Head(StopOnShutdown::new(Store)));
                    Creations::one(CreateChild::birth(store, store_child()))
                        .and(CreateChild::birth(alternate_store, store_child()))
    ...
    fn rejected_creation_remains_a_typed_planning_failure() {
        ...
        let mut active = shutdown_after_children(Application::complete(children))
            ...
            .unwrap()
            .behavior;
        let rejected = CreationResolved::rejected(
            children.store,
            CreationKind::Birth,
            behavior::CreationRejection::EnvironmentFailed,
        );
        let result = active.on_path::<_, Inside<Inside<Here>>>(rejected);
    ...
    fn configured_children_are_total_and_unique_before_any_observation_is_emitted() {
        let children = ApplicationChildren::issue();
        let missing = shutdown_after_children(Application::missing_store(children))
            .shutdown_phase(StoreRole)
            .shutdown_phase(GatewayRole)
            .finish()
            .initialize();
        ...
        let duplicate = shutdown_after_children(Application::duplicate_store(children))
            .shutdown_phase(StoreRole)
            .shutdown_phase(GatewayRole)
            .finish()
            .initialize();
    ...
    #[test]
    fn mismatched_and_stale_creation_resolutions_are_returned_complete() {
        let children = ApplicationChildren::issue();
        let mut mismatched = shutdown_after_children(Application::complete(children))
            .shutdown_phase(StoreRole)
            .shutdown_phase(GatewayRole)
            .finish()
            .initialize()
            ...
            .behavior;
        let observed =
            CreationResolved::replacement(children.alternate_store, children.store, MailAddr(191));
        ...
        let mut stale = shutdown_after_children(Application::complete(children))
            .shutdown_phase(StoreRole)
            .shutdown_phase(GatewayRole)
            .finish()
            .initialize()
        ...
        assert!(
            stale
                .on_path::<_, Inside<Inside<Here>>>(CreationResolved::birth(
                    children.store,
                    MailAddr(101),
                ))
                .is_ok()
        ...
        assert!(
            stale
                .on_path::<_, Inside<Here>>(CreationResolved::birth(
                    children.gateway,
                    MailAddr(102),
                ))
                .is_ok()
        );
        let observed = CreationResolved::birth(children.gateway, MailAddr(202));
        let Err(ShutdownCoordinatorError::Behavior(
            ChildShutdownPlanError::UnexpectedCreationResult {
                position,
                expected,
...
where
    B: Behavior,
    <B::Birth as BirthMode>::Child: ChildOccurrenceProduct<AvailableChildren>,
{
    ChildShutdownPhases {
        application,
...
/// Shape of the child product consumed by phase declarations.
...
impl ChildOccurrenceShape for AvailableChildren {
    type Empty = NoChildren;
    type Member<Occurrence, Child: Behavior, Tail> = Unassigned<Child, Tail>;
}
...
/// Shape of the coordinator's heterogeneous target sum.
...
pub type TargetsFor<B> =
    ChildOccurrences<<<B as Behavior>::Birth as BirthMode>::Child, ShutdownTargets<B>>;

/// Complete role-availability product derived from direct births.
pub type AvailableFor<B> =
    ChildOccurrences<<<B as Behavior>::Birth as BirthMode>::Child, AvailableChildren>;
...
impl ChildStatus {
    fn expectation(self) -> ChildCreationExpectation {
        match self {
            Self::NotRequested => ChildCreationExpectation::NotRequested,
            Self::Awaiting { creation, kind } => {
                ChildCreationExpectation::Awaiting { creation, kind }
            }
            Self::Established { creation } => ChildCreationExpectation::Established { creation },
        }
    }
}

enum Planning {
    Collecting(Vec<ChildStatus>),
    Reported,
}
...
impl ChildOccurrenceShape for PlanningChildren {
    type Empty = PlannedEnd;
    type Member<Occurrence, Child: Behavior, Tail> = PlannedChild<Occurrence, Child, Tail>;
}
...
trait PlanChildren<Node, A: Address, Event, Sends> {
    ...
    fn observe(
        ...
        requested: &mut Vec<(usize, CreationId, CreationKind)>,
    );
    ...
    Sends: SendEffects,
{
    ...
    fn observe(
        ...
        _: CreationKind,
        _: &mut Self::Sends,
        _: &mut Vec<(usize, CreationId, CreationKind)>,
    ) {
        match *child {}
    }
    ...
    Child::Protocol: Protocol<Addr = A>,
{
    ...
    type Sends = SendLayer<InterpreterRequests<ObserveCreation<Child::Protocol, Position>>, Sends>;
    const COUNT: usize = 1;
    ...
    fn observe(
        ...
        sends.owned.send(ObserveCreation::new(creation));
        requested.push((Position::INDEX, creation, kind));
    }
    ...
    TailPlan: PlanChildren<Tail, A, Event, Sends>,
{
    ...
    type Sends =
        SendLayer<InterpreterRequests<ObserveCreation<Head::Protocol, Position>>, TailPlan::Sends>;
    ...
    fn observe(
        ...
        match child {
            ...
            ChildChoice::Tail(tail) => {
                TailPlan::observe(tail, creation, kind, &mut sends.inner, requested);
            }
        }
    }
    ...
    Earlier: BuildShutdownPlan<B, Targets>,
{
    fn build(
        ...
        phases.push(vec![Targets::shutdown_target_at(*creation)]);
        Ok(phases)
    }
}
...
where
    ...
    Node: ChildOccurrenceProduct<ShutdownTargets<B>, Product = Targets>
        + ChildOccurrenceProduct<PlanningChildren, Product = Shape>,
    ...
    Planned: SendEffects + behavior::SendsFor<Events>,
{
    fn wrap_initialization(
        ...
        if Shape::COUNT != 0 {
            let mut next = vec![ChildStatus::NotRequested; Shape::COUNT];
            for (position, creation, kind) in requested {
                let Some(state) = next.get_mut(position) else {
                    return Err(ChildShutdownPlanError::DuplicateCreation { position });
                };
                if !matches!(state, ChildStatus::NotRequested) {
                    return Err(ChildShutdownPlanError::DuplicateCreation { position });
                }
                *state = ChildStatus::Awaiting { creation, kind };
            }
            if let Some(position) = next
                .iter()
                .position(|child| matches!(child, ChildStatus::NotRequested))
            {
    ...
    fn creation_resolved(
        ...
        let ChildStatus::Awaiting { creation, kind } = state else {
            return Err(ChildShutdownPlanError::UnexpectedCreationResult {
                position,
        ...
        if result.creation != creation || result.kind != kind {
            return Err(ChildShutdownPlanError::UnexpectedCreationResult {
                position,
                expected: ChildCreationExpectation::Awaiting { creation, kind },
                observed: result,
            });
        }
        let established = match result.result {
            Ok(_) => ChildStatus::Established { creation },
            Err(_) => {
        ...
        if !next
            .iter()
            .all(|child| matches!(child, ChildStatus::Established { .. }))
        {
            self.planning = Planning::Collecting(next);
