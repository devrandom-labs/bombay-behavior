...
| Guardian / application root | Preserve the wrapped bootstrap topology and define normal root shutdown without recovery policy; heterogeneous root children use a closed creation-only `#[behavior::births]` sum | Transactional activation, exhaustive concrete `InstallBirth`, address publication, child retention and retirement | Bombay Driver, `System` and incarnation lifecycle | pure `Guardian` and heterogeneous birth contract realized; Bombay installer/façade integration remains partial |
...
| Backoff supervisor / retrying child | Restart decision, checked delay, pending replacement batch and exact timer generation | Timers, child installation and terminal observation | Timers, Observe, Bombay | pure `BackoffSupervisor` realized; façade/runtime integration partial |
| Dynamic supervisor | Typed start/stop/replace/query admission and explicit installing/available/stopping/replacing/retired slots | Fresh proxy installation, creation resolution, child shutdown and exact terminal publication | Bombay, Address, Observe | pure `DynamicSupervisor` realized; child-shutdown interpreter lane remains an integration gap |
