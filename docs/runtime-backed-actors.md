...
| Guardian / application root | Preserve the wrapped bootstrap topology and define normal root shutdown without recovery policy | Transactional activation, address publication, child retention and retirement | Bombay Driver, `System` and incarnation lifecycle | pure `Guardian` template realized; application façade integration remains partial |
