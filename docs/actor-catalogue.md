...
| `supervision` | `Supervisor` | fleet/incarnation/restart-budget sums and `SupervisionEvent` | `SupervisorSends`, `SupervisorError`, `SupervisionFailure` | fresh installation, Observe | unit, model, exhaustive, properties, fuzz | `bombay::actors::Supervisor` |
| `supervision` | `WorkerPool` / `KeyedWorkerPool` | assignment, worker phase, interruption policy, keyed events | `PoolActions`, `PoolSends`, `PoolError`, `PoolRejection` | fresh installation, Observe | unit, independent ownership model, properties, fuzz | `bombay::actors::{Pool, KeyedPool}` |
