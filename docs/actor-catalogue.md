...
| `supervision` | `BackoffSupervisor` | `Supervisor` plus checked one-based backoff attempts, timer generations and pending replacement batches | structural timer layer over `SupervisorSends`, typed configuration/overflow/collision errors | Timers, fresh installation, Observe | unit over policy bounds, delayed release, stale/colliding timers and repeated failures | `bombay::actors::{BackoffSupervisor, Backoff}` |
