...
fn at<T: Behavior>(behavior: Spec<T>, when: Instant) -> Spec<behaviorpass::At<T>> {
...
async fn all_wrapper_permutations_preserve_init_protocol_nesting() {
    ...
    assert_eq!(i1.sends.own[0].message.at, second);
    assert_eq!(i1.sends.inner.own[0].message.peer, PEER);
    assert_eq!(i1.sends.inner.inner.own[0].message.at, first);
    ...
    assert_eq!(i2.sends.inner.own[0].message.at, second);
    assert_eq!(i2.sends.inner.inner.own[0].message.at, first);
    ...
    assert_eq!(i3.sends.own[0].message.at, second);
    assert_eq!(i3.sends.inner.own[0].message.at, first);
    ...
    assert_eq!(i4.sends.inner.own[0].message.at, first);
    assert_eq!(i4.sends.inner.inner.own[0].message.at, second);
    ...
    assert_eq!(i5.sends.own[0].message.at, first);
    assert_eq!(i5.sends.inner.own[0].message.peer, PEER);
    assert_eq!(i5.sends.inner.inner.own[0].message.at, second);
    ...
    assert_eq!(i6.sends.own[0].message.at, first);
    assert_eq!(i6.sends.inner.own[0].message.at, second);
...
async fn stash_layer_contributes_no_init_sends() {
    ...
    assert_eq!(initial.sends.own[0].message.at, due);
...
async fn environment_lanes_bypass_stash_while_user_lane_is_intercepted() {
    ...
    assert!(behavior.behavior().inner().inner().inner().state().seen.is_empty());
}
...
async fn watch_reaction_reinvokes_on_each_death_and_fold_continues() {
    ...
    let actions = behavior.step(UserEvent::user(MailAddr(2), 9)).await.unwrap();
