...
impl State<Never, Child, Boom> for FailingParent {
    type Addr = MailAddr;
    type Msg = u64;
    ...
    fn handle(
        ...
        _from: MailAddr,
        _message: u64,
    ) -> Acted<MailAddr, Never, Vec<Delivery<MailAddr, Never>>, Child, Boom> {
        if self.fail {
            self.fail = false;
            return Err(Boom);
        }
...
async fn fsm_error_mid_drain_drops_the_unprocessed_batch() {
    ...
    let mut machine = Fsm::new(Vec::new(), Phase::P0, |phase, _seen: &mut Vec<u64>, id: &u64| {
        match (phase, id % 4) {
...
async fn at_reaction_error_consumes_the_timer() {
    ...
    let mut behavior = Spec::new(FailingParent { fail: true })
        .at(Some(due), |_| Err(Boom));
...
async fn watch_reaction_error_propagates() {
    ...
    let mut behavior = Spec::new(FailingParent { fail: true }).watch(peer, |_b, _p, _o| {
        Err(Boom)
    });
    behavior.init().await.unwrap();
