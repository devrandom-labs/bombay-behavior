...
use behaviorpass::{
    ...
    Recipient, RestartPolicy, Route, SendAlgebra, SendProduct, State, Step, Strategy,
    Supervising, SupervisionEvent, User, UserEvent,
};
...
type Supervisor = Supervising<Base<EchoingParent, u64, Child, Never>, Child>;
...
async fn driver_full_stack_mixed_lanes_stop_on_peer_death() {
    ...
    let mut behavior = Spec::new(EchoingParent { seen: Vec::new() })
        .stash(|m| if m % 3 == 2 { StashRoute::Stash } else { StashRoute::Deliver })
    ...
    let mut mailbox = Mailbox::new([
        SupervisionEvent::Inner(AtEvent::Inner(WatchEvent::Inner(User::user(MailAddr(9), 1)))),
        SupervisionEvent::Inner(AtEvent::Inner(WatchEvent::Inner(User::user(MailAddr(9), 5)))),
        SupervisionEvent::Inner(AtEvent::Reached(TimeReached { id: AtId(0), at: due })),
        SupervisionEvent::Inner(AtEvent::Inner(WatchEvent::PeerStopped(PeerStopped {
            peer,
            outcome: Err(Crash::Failed),
        }))),
        SupervisionEvent::Inner(AtEvent::Inner(WatchEvent::Inner(User::user(MailAddr(9), 7)))),
    ]);
