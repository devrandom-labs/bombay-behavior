...
async fn empty_mailbox_still_observes_initialization_exactly_once() {
    ...
    assert_eq!(trace.sends.own[0].message.at, due);
}
...
async fn wrapper_orderings_preserve_both_initial_protocols() {
    ...
    assert_eq!(first.sends.inner.own[0].message.at, due);
    ...
    assert_eq!(second.sends.own[0].message.at, due);
}
...
async fn proxy_forwards_only_to_the_current_fresh_generation() {
    ...
    let before = proxy
        ...
        .await
        .unwrap();
    assert_eq!(before.sends[0].to.route(), Route::Child(0));
    ...
    let after = proxy
        .step(User::user(MailAddr(0), ProxyCommand::Forward(6)))
        .await
        .unwrap();
    assert_eq!(after.sends[0].to.route(), Route::Child(1));
}
...
async fn restart_window_boundary_is_inclusive() {
    ...
    let first = SupervisionEvent::ChildStopped(ChildStopped {
        nonce: 0,
    ...
    let edge = SupervisionEvent::ChildStopped(ChildStopped {
        nonce: 0,
        outcome: Err(Crash::Failed),
