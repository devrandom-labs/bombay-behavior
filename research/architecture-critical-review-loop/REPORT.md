...
5. **Access limitations (explicit).** Paywalled and inaccessible items:
   ...
   Baker-Hewitt IFIP 1977 was read through its MIT Working Paper 134 draft
   on DSpace (actor concept and activation-ordering laws extracted);
...
| Baker-Hewitt WP-134 1976 / IFIP 1977 (read from the MIT DSpace draft: actor concept + activation-ordering laws) | actors only; messenger receipt as the sole event kind; acquaintances by birth or acquisition | axiomatic laws on computation histories | Law of Discreteness (no Zeno machines); finite immediate successors; at most one immediate predecessor; unique initial event | causal activation order + finite branching per transition = LAW at history level; the fold enforces finite branching structurally; cross-lane order stays interpreter policy |
...
| Talcott TACS 1997 (full text read from the archived author copy) + HOSC 2000; the AFP 1996 'An Actor Algebra' label resolved to these works (it does not appear in Talcott's own publication list) | event diagrams for open systems; interaction diagrams; interaction paths | three semantic models with algebras: parallel composition, hiding, renaming | component-algebra homomorphism | configuration-level composition laws; Bombay's typed sums + wrappers play the hiding role at type level (derived) |
| Mason-Talcott 1997 (full text read from the authors' archived manuscript: uL with disable-guards -> kL kernel) | user language with synchronization constraints; kernel = CBV lambda + create/send/ready | labelled transition semantics + interaction semantics | Theorem (u2k): translation preserves interaction semantics exactly | the primary proof that synchronization constraints are derived — closes the selective-receive evidential chain; crisp three-source acquaintance law (creation, message, creator-given) |
...
| Kumar-Sen-Meseguer-Agha FMOODS 2003 (full text read); PMaude QAPL 2005/ENTCS 2006 (abstract + record); DoS FCS 2005 (record) | probabilistic rewrite theories; actor PMaude module; QuaTEx | rewriting-logic specification of probabilistic object/actor systems; statistical analysis | probability = specification/observation layer = INTERPRETER/environment; no behavior primitive |
...
| Charalambides-Palmskog-Agha 2017/2019 (full text read from the IDEALS open collection) | typestates tag actor names with multisets of required message types; requirements fulfilled in scope or delegated | progress as persistent configuration property: every trace ends with all generated requirements satisfied; limitation: no termination for cyclic messaging, safety out of scope | liveness-by-types = static layer over the nucleus; no primitive impact |
...
| Honda CONCUR 1993 (record-level; no open copy) and Honda-Vasconcelos-Kubo ESOP 1998 (full text read from the author's copy); Honda-Yoshida-Carbone POPL 2008/JACM 2016 (records) — adversarial probe | session types: fresh private channel per dyadic session; value passing, label branching, delegation | ML-style duality discipline guarantees two-party compatibility | session duality is NOT an actor-model law; dynamic acquaintance vs static channels mismatch; the session campaign's result stands: finite-state sequencing derived, Honda-style typing only partially represented, no demonstrated basis gap |
