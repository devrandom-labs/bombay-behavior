...
> Supervised Worker campaign is preserved separately. See
> `research/README.md` for chronology.
...
This campaign owns the attempt-by-attempt breakdown. The architecture campaign
owns the system-wide capability inventory and records only the resulting
classification.

This campaign is a falsification probe for the candidate behavior calculus. It
must return a concrete derivation or obstruction to the architecture campaign;
it must not propose a parallel primitive basis or infer global completeness
from this single protocol.
