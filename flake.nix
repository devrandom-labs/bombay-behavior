{
  description = "behaviorpass — capability-machinery concision harness (bombay card #298)";
  inputs = {
    ...
    utils.lib.eachDefaultSystem (
      ...
      in
      {
        ...
        devShells.miri = pkgs.mkShell {
          ...
          shellHook = ''
            echo "behaviorpass MIRI shell — nightly, on-demand only."
            echo "  cargo miri setup"
            echo "  MIRIFLAGS=\"-Zmiri-many-seeds=8\" cargo miri test -p behaviorpass"
          '';
        };
      }
    );
